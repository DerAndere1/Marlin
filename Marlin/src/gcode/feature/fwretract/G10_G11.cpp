...
void GcodeSuite::G10() {
  ...
  #if ANY(CNC_COORDINATE_SYSTEMS, HAS_TOOL_LENGTH_COMPENSATION)
    uint8_t index,
            offset_type;
        ...
        switch (offset_type) {
          ...
          case 1: // Sets the tool offset. Intended to work as in LinuxCNC.
            LOOP_NUM_AXES(i) {
              if (parser.seen(axis_codes[i])) {
                ...
                if (i == Z_AXIS) {
                  motion.hotend_offset[index][i] = -axis_value;
                }
                else {
                  motion.hotend_offset[index][i] = axis_value;
                }
          ...
          case 10:
            LOOP_NUM_AXES(i) {
              if (parser.seen(axis_codes[i])) {
                const float axis_value = parser.value_axis_units((AxisEnum)i);
                motion.hotend_offset[index][i] = motion.position[i] - axis_value;
              }
            }
            break;
        }
            ...
            LOOP_LOGICAL_AXES(i) {
              if (parser.seenval(axis_codes[i])) {
                if (TERN1(HAS_EXTRUDERS, i != E_AXIS)) {
                  ...
                  motion.workspace_offset[i] = -axis_shift;
                }
              }
            }
            break;
          ...
          case 20:
            LOOP_LOGICAL_AXES(i) {
              if (parser.seenval(axis_codes[i])) {
                if (TERN1(HAS_EXTRUDERS, i != E_AXIS)) {
                  const float axis_value = parser.value_axis_units((AxisEnum)i);
                  motion.workspace_offset[i] = axis_value - motion.position[i];
                }
              }
            }
            break;
