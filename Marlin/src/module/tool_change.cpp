...
#include "../core/debug_out.h"
  ...
  inline void switching_toolhead_tool_change(const uint8_t new_tool, bool no_move/*=false*/) {
    ...
    motion.position.x = SUM_TERN(HAS_HOTEND_OFFSET, placexpos, motion.hotend_offset[motion.extruder].x);
    ...
    motion.position.y = SUM_TERN(HAS_HOTEND_OFFSET, SWITCHING_TOOLHEAD_Y_POS, motion.hotend_offset[motion.extruder].y) - (SWITCHING_TOOLHEAD_Y_CLEAR);
    ...
    fast_line_to_current(Y_AXIS);  // move Y in front of the toolhead dock
      ...
      motion.position.y = SUM_TERN(HAS_HOTEND_OFFSET, SWITCHING_TOOLHEAD_Y_POS, motion.hotend_offset[motion.extruder].y);
      ...
      motion.position.z = SUM_TERN(HAS_HOTEND_OFFSET, SWITCHING_TOOLHEAD_Z_POS, motion.hotend_offset[motion.extruder].z);
      ...
      #endif
      motion.position.y = SUM_TERN(HAS_HOTEND_OFFSET, SWITCHING_TOOLHEAD_Y_POS, motion.hotend_offset[motion.extruder].y) - (SWITCHING_TOOLHEAD_Y_SECURITY);
    ...
    motion.position.x = SUM_TERN(HAS_HOTEND_OFFSET, grabxpos, motion.hotend_offset[motion.extruder].x);
    ...
    #if defined(SWITCHING_TOOLHEAD_Z_CLEAR)
      #if HAS_HOTEND_OFFSET
        motion.position.z = SWITCHING_TOOLHEAD_Z_POS + motion.hotend_offset[motion.extruder].z - motion.hotend_offset[new_tool].z + (SWITCHING_TOOLHEAD_Z_CLEAR);
      #else
        motion.position.z = SWITCHING_TOOLHEAD_Z_POS + (SWITCHING_TOOLHEAD_Z_CLEAR);
      #endif
      DEBUG_POS("Move back Z clear", motion.position);
  ...
  if (TERN0(MAGNETIC_SWITCHING_TOOLHEAD, new_tool == motion.extruder))
    ...
    if (new_tool != old_tool || TERN0(PARKING_EXTRUDER, extruder_parked)) { // PARKING_EXTRUDER may need to attach old_tool when homing
      ...
      // Raise to safe Z
        ...
        if (can_move_away && TERN1(TOOLCHANGE_PARK, toolchange_settings.enable_park)) {
          motion.position.z = SUM_TERN(HAS_HOTEND_OFFSET, (SAFE_TOOLCHANGE_START_Z), motion.hotend_offset[old_tool].z);
          TERN_(HAS_SOFTWARE_ENDSTOPS, NOMORE(motion.position.z, motion.soft_endstop.max.z));
          fast_line_to_current(Z_AXIS);
          planner.synchronize();
        }
      ...
      if (should_move) {
        ...
        if (can_move_away) {
            ...
            #if defined(SAFE_TOOLCHANGE_START_Z)
              if (TERN1(TOOLCHANGE_PARK, toolchange_settings.enable_park)) {
                motion.blocking_move_to_z(SUM_TERN(HAS_HOTEND_OFFSET, (SAFE_TOOLCHANGE_START_Z), motion.hotend_offset[new_tool].z), planner.settings.max_feedrate_mm_s[Z_AXIS]);
              }
            #endif
