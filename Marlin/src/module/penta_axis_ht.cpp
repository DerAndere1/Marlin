...
xyz_pos_t native_to_joint(const xyz_pos_t &native) {
  ...
  xyz_pos_t pos = NUM_AXIS_ARRAY(
                    DIFF_TERN(HAS_HOTEND_OFFSET, native.x, motion.hotend_offset[active_extruder].x),
                    DIFF_TERN(HAS_HOTEND_OFFSET, native.y, motion.otend_offset[active_extruder].y),
                    native.z,
                    native.i,
                    native.j
                  );

  const float pivot_length = DIFF_TERN(HAS_HOTEND_OFFSET, mrzp_offset_z, motion.hotend_offset[active_extruder].z);
  const float i_rad = RADIANS(pos.i);
    ...
    const xyz_pos_t joints_pos = NUM_AXIS_ARRAY(
      xyr * cos(xytheta),
      xyr * sin(xytheta),
      DIFF_TERN(HAS_HOTEND_OFFSET, pos.z, motion.hotend_offset[active_extruder].z),
      pos.i,
      pos.j
    );
  #endif
