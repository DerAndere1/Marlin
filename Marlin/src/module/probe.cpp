...
xyz_pos_t Probe::run_probe(const bool sanity_check/*=true*/, const xyz_pos_t target/*=Z_PROBE_LOW_POINT*/, const_float_t z_clearance/*=Z_TWEEN_SAFE_CLEARANCE*/, const bool probe_straight, const uint8_t move_value) {
  ...
  const xyz_pos_t offs = SUM_TERN(HAS_HOTEND_OFFSET, -offset, motion.hotend_offset[motion.extruder]);
  ...
  auto try_to_probe = [&](PGM_P const plbl, const xyz_pos_t target_point, const feedRate_t fr_mm_s, const bool scheck) -> bool {
    ...
    const bool probe_fail = probe_to_target(target_point, fr_mm_s, move_value),              // No probe trigger?
               early_fail = (sanity_check && motion.position.z > target.z + error_tolerance); // Probe triggered too high?
    #if ENABLED(DEBUG_LEVELING_FEATURE)
      if (DEBUGGING(LEVELING) && (probe_fail || early_fail)) {
        DEBUG_ECHOPGM(" Probe fail! - ");
        if (probe_fail) DEBUG_ECHOLNPGM("No trigger.");
        if (early_fail) DEBUG_ECHOLNPGM("Triggered early (above ", target.z + error_tolerance, "mm)");
      }
    #else
      UNUSED(plbl);
    #endif
  ...
  // Double-probing does a fast probe followed by a slow probe
      ...
      if (probe_straight) {
        const float targ1 = motion.position;
        if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("1st Probe Z:", targ1);
      }
      else {
        const float targ1 = DIFF_TERN(HAS_DELTA_SENSORLESS_PROBING, motion.position.z, largest_sensorless_adj);
        if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("1st Probe Z:", targ1);

      }

      // Raise to give the probe clearance
      motion.do_z_clearance(targ1 + (Z_CLEARANCE_MULTI_PROBE), false);

  #elif Z_PROBE_FEEDRATE_FAST != Z_PROBE_FEEDRATE_SLOW
    if (probe_straight) {
      if(!probe_to_target(probe_target_point, motion.z_probe_fast_mm_s, move_value)) {
      ...
      const float z = (Z_CLEARANCE_DEPLOY_PROBE) + 5.0f + _MAX(offs.z, 0.0f);
  ...
  #endif
    {
      ...
      if (try_to_probe(PSTR("SLOW"), probe_target_point, motion.z_probe_slow_mm_s, sanity_check)) return nan_pos;
  ...
  #elif TOTAL_PROBING == 2
    if (probe_straight) {
      const xyz_pos_t targ2 = motion.position;
    }
    else {
      const float targ2 = DIFF_TERN(HAS_DELTA_SENSORLESS_PROBING, motion.position.z, largest_sensorless_adj);
    }
    ...
    if (probe_straight) {
      const xyz_pos_t measured = NUM_AXIS_LIST(
        (motion.position.x * 3.0f + targ1.x * 2.0f) * 0.2f,
        (motion.position.y * 3.0f + targ1.y * 2.0f) * 0.2f,
        (motion.position.z * 3.0f + targ1.z * 2.0f) * 0.2f,
        (motion.position.i * 3.0f + targ1.i * 2.0f) * 0.2f,
        (motion.position.j * 3.0f + targ1.j * 2.0f) * 0.2f,
        (motion.position.k * 3.0f + targ1.k * 2.0f) * 0.2f,
        (motion.position.u * 3.0f + targ1.u * 2.0f) * 0.2f,
        (motion.position.v * 3.0f + targ1.v * 2.0f) * 0.2f,
        (motion.position.w * 3.0f + targ1.w * 2.0f) * 0.2f      
      );
    }
    else {
      const float measured = (motion.position * 3.0f + targ1 * 2.0f) * 0.2f;
  #else

    // Return the single probe result
    const xyz_pos_t measured = motion.position;

  #endif
  
  #if HAS_HOTEND_OFFSET
    return measured - motion.hotend_offset[motion.extruder];
  #else
    return measured;
  #endif
}
...
float Probe::run_z_probe(const bool sanity_check/*=true*/, const_float_t z_min_point/*=Z_PROBE_LOW_POINT*/, const_float_t z_clearance/*=Z_TWEEN_SAFE_CLEARANCE*/) {
  ...
  const xyz_pos_t target_coordinates = { NUM_AXIS_LIST(current_position.x, 
                                                      motion.position.y, 
                                                      z_min_point, 
                                                      motion.position.i, 
                                                      motion.position.j, 
  ...
  const xyz_pos_t result = run_probe(sanity_check, target_coordinates, z_clearance, false, 0);
  return result.z;
}
...
xyz_pos_t Probe::probe_straight(
  ...
  DEBUG_SECTION(log_probe, "Probe::probe_straight", DEBUGGING(LEVELING));
  ...
  xyz_pos_t measured;
    ...
    if (isnan(measured.z)) {
      ...
      // This would also benefit from the contemplated "Audio Alerts" feature.
      stow();
      SERIAL_ECHOLNPGM_P("deploy failed");
      LCD_MESSAGE(MSG_LCD_PROBING_FAILED);
      #if DISABLED(G29_RETRY_AND_RECOVER)
        SERIAL_ERROR_MSG(STR_ERR_PROBING_FAILED);
      #endif
...
bool Probe::probe_to_target(const xyz_pos_t pos, const_feedRate_t fr_mm_s, const uint8_t move_value) {
